...
# ───────────────────────── УСТАНОВКА BROWSERS ─────────────────────────
# ВАЖНО: способ установки РОВНО как в твоём коде 1.
...
def ensure_playwright_browsers_with_retry(max_attempts: int = 8) -> None:
    """
    Команда установки НЕ меняется (та же самая, как в коде 1),
    просто повторяем попытки с backoff, потому что на Render иногда рвётся CDN download.
    """
    base = 3.0
    for i in range(1, max_attempts + 1):
        ...
        if i == max_attempts:
            return
        sleep_s = min(90.0, base * (2 ** (i - 1)))
        # небольшой jitter, чтобы не повторять в одинаковые моменты
        sleep_s = sleep_s + random.uniform(0, 1.5)
        log.info("Retrying Playwright install in %.1fs (attempt %d/%d)...", sleep_s, i + 1, max_attempts)
        time.sleep(sleep_s)
...
async def worker() -> None:
    # 1) Установка браузеров (ровно тем же способом, что в коде 1)
    ensure_playwright_browsers_with_retry(max_attempts=8)
                ...
                if page is None:
                    ...
                    try:
                        ...
                        if _is_missing_browser_error(e):
                            # Браузера реально нет — повторяем установку тем же способом
                            ensure_playwright_browsers_with_retry(max_attempts=8)
                            browser, context, page = await open_browser(pw)
                        else:
                            raise
